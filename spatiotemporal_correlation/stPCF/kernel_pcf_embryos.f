...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      integer n,ns,nt,np,edg,is,it,iu,iv,nv,ks,kt,counts,out_unit
      double precision pcfhat(ns,nt), two, hs, ht, lambda(n)
      dimension x(n),y(n),txy(n),xp(np+1),yp(np+1),s(ns),t(nt),label(n)
                ...
                ti=txy(i)
                    ...
                    do j=1,n
                        ...
                        if ((j.ne.i).and.(label(i).eq.label(j))) then
                            ...
                            end if ! nonzero kernel

                        end if ! labels the same
                    end do
                end do
          end do
      end do
