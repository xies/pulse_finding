...
function submit_randomization{
	...
	CMD="matlab -nojvm -nodisplay -nodesktop -nosplash -r \"addpath(genpath('~/Code'));batch_randomize('${PULSE_DIR}/${INPUT_PULSES}','${MAT_OUT}')\""
...
do
	submit_randomization twist.mat $i
done
